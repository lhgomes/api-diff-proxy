...
def test_proxy_controls_its_own_comparison_id_header(monkeypatch):
    ...
    async def fake_call(client, base_url, request, body):
        return httpx.Response(200, content=b"ok", headers={"X-API-Diff-ID":"backend-value"}), 1.0, None
    monkeypatch.setattr(module, "call_backend", fake_call)
    with TestClient(module.app) as client:
        response = client.get("/api/test")
    assert response.headers["X-API-Diff-ID"] != "backend-value"
