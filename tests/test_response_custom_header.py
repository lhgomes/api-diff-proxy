...
def test_primary_custom_response_header_is_preserved(monkeypatch):
    ...
    async def fake_call(client, base_url, request, body):
        return httpx.Response(200, content=b"ok", headers={"x-business-value":"123"}), 1.0, None
    monkeypatch.setattr(module, "call_backend", fake_call)
    with TestClient(module.app) as client:
        response = client.get("/api/test")
    assert response.headers["x-business-value"] == "123"
