...
def test_primary_application_error_is_returned_not_converted_to_proxy_error(monkeypatch):
    ...
    async def fake_call(client, base_url, request, body):
        return httpx.Response(500, json={"error":"backend"}), 1.0, None
    monkeypatch.setattr(module, "call_backend", fake_call)
    with TestClient(module.app) as client:
        response = client.get("/api/test")
    assert response.status_code == 500
    assert response.json() == {"error":"backend"}
